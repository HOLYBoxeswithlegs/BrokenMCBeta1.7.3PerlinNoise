...
public class NoiseGeneratorPerlin extends NoiseGenerator {
    ...
    static class NoiseParams {
        ...
        NoiseParams() {
            offset = 0.0f;
            scale = 0.3f; // Further reduced amplitude to limit height variations
            spread = new Vector3(600.0f, 600.0f, 600.0f); // Increased Z spread to 600 for smoother vertical transitions
            seed = 0;
            octaves = 2; // Reduced to 2 to avoid Far Lands-like artifacts
            persist = 0.6f;
            lacunarity = 2.0f;
            flags = 2; // NOISE_FLAG_EASED
        }
    }
    ...
    public void func_805_a(double[] var1, double var2, double var4, double var6, int var8, int var9, int var10,
        ...
        if (var9 == 1) {
            ...
            for (int i = 0; i < var8 * var10; i++) {
                var1[i] += result[i] * 16.0 / var17; // Reduced scaling to limit height
            }
        } else {
